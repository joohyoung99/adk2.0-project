...
사용자 자연어 입력, PostgreSQL에 저장된 냉장고 재고/선호 정보를 결합해 **레시피 추천**, **조건부 분기**, **대체재 기반 재시도**를 수행합니다. DB 후보가 부족한 경우 branch agent가 LLM 지식으로 직접 레시피를 제안합니다. 레시피 이미지는 별도의 `image_search_agent`가 Google Search로 탐색합니다.
...
입력 문장을 구조화한 뒤, 사용자 재고와 선호 정보를 조회하고, DB 기반으로 후보 레시피를 1차 탐색합니다. 현재 재료 상태에 따라 **즉시 조리 가능 / 대체재 필요 / 장보기 필요**로 분기하며, 각 branch agent는 DB 결과를 우선 활용하되 **부족하거나 없는 경우 LLM 지식을 바탕으로 직접 레시피를 제안**합니다. 부족한 재료가 있는 경우에는 동적 루프를 통해 대체재를 탐색하고 재평가합니다. 최종 추천 후에는 `image_search_agent`가 Google Search로 음식 이미지를 탐색합니다.
...
[save_recommendation_log]   ◀── Function Node: 결과 저장
  ▼
[image_search_agent]        ◀── LLM Node + google_search: 레시피 이미지 탐색
  ▼
END
```
...
11. **Image Search Agent (LLM + google_search)**
    추천된 대표 레시피명으로 Google Search를 호출해 음식 이미지 URL을 탐색합니다.
...
### 10. `image_search_agent` — LLM Node (+ google_search)

추천된 레시피명과 주요 재료로 Google Search를 호출해 음식 이미지 URL을 탐색합니다. 직접 표시 가능한 HTTPS 이미지 URL을 우선 반환하고, 불확실한 경우 출처 페이지 URL을 반환합니다.

### 11. `RecipeRecoveryWorkflow` — Dynamic BaseNode
...
### Image Search Agent 내장 Tool

* `google_search` — ADK 내장 tool. `image_search_agent`가 추천 레시피 이미지 URL 탐색에 사용
...
* **이미지 탐색 분리**: 레시피 생성 이후 `image_search_agent`가 Google Search로 음식 이미지를 별도 탐색
