...
## 구매처 비교 위임
추가 구매가 필요한 재료가 있으면 먼저 compare_market_prices_for_missing 도구를 호출하세요.
...
- 도구 결과가 있으면 그 결과를 4~5번 섹션에 우선 반영하세요.
- compare_market_prices_for_missing 또는 MarketPriceAgent 의 JSON 원문을 최종 답변으로 그대로 출력하지 마세요.
- 도구 결과는 반드시 사용자가 읽기 쉬운 한국어 레시피/장보기 문장으로 풀어 쓰세요.
...
## 작업 순서
1. 반드시 compare_market_prices 도구를 호출한다.
2. 도구 결과의 offers, recommended_market, warnings 를 그대로 검토한다.
3. catalog에 없는 재료는 observed_price=null, confidence="low"로 처리된 결과를 사용한다.
